...
of different Hilbert spaces, each one corresponding to a different number of 
particles in the system. Allowing the description of quantum systems with a 
variable number of particles. It is essential in quantum field theory, 
where the number of particles is not conserved and processes such as particle 
creation and annihilation occur.
...
and the terms \( \mathbb{H}^{\otimes n} \) represents the Hilbert space for 
\( n \)-particle states.
...
defining the canonical conjugate momenta,
...
momentum conservation. As a result, the vacuum in FF is trivial or "empty": 
...
of quantum field theory. By considering a series of unitary transformations applied to the
...
legs, and 1 vertex. Being an order 1 term, with the corresponding diagrams shown in
...
1 gluon is created, or 2 gluons are annihilated, and 2 gluons are created. But since 
we are interested in building the diagrams at each order with diagrams of lower
order, it's convenient to consider the 4 gluon vertex as a combination of two 3 gluon
vertices. Meaning that for the process where one gluon goes to 3 gluons, we consider 
the process as, one gluon being annihilated, and 2 gluons are created, then one of the
...
The fascinating term is $H_{\sbrackets{\partial A A}^2}$ in
...
pair of 3 gluon vertices, similar to $H_{A^4}$. The main difference is that
...
This process is usually represented in by a perpendicular
...
This way simplifying the number of diagrams that can be obtained at higher orders,
...
Referring to the problems mentioned in the section \ref{sec:applied_gluons}, the diagrams 
