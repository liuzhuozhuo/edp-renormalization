...
"""
Let's indicate the shape of the array for the different type of arrays that it will be used.
- points (2D arrays): arrays that contain the different coordinate points in a 2D plane where connections can be made.
    - 1st dim: contain the different points --> len(points) indicate the number of points
    - 2nd dim: the x, y coordinates of the plane. --> len(points[i]) = 2 for all i

- paths (3D arrays): arrays that define how the connections between points are made. 
    - 1st dim: indicate the type of particle considered --> len(paths) = 2 in this case, in general indcate the number of types of particle.
    - 2nd dim: contain the different conections made
    - 3rd dim: contain the index for the 2 connected point
"""

# Used to detect 2 loop sin the diagrams
...
def represent_diagram (points, all_paths, index = False, directory = "", colors = ["tab:blue", "tab:red", "black"], line = ["solid", "solid", "photon"], number = 0):
    ...
    """
    # Clumsy fix to the proble that some times due to the dimensions asigned to the 
    # numpy arrays, there may be cases where all 0 arrays are passed as argument
...
def unique_values(array):
    """
    Function used to detect unique values in a ndarray, in this case is used to detect ins/outs particles in the path array.
    """
...
def how_connected( max_connections, n_connections, n_1, n_2):
    """
    Generate all possible combinations of connections between two diagrams. This will be used in the next function to generate the paths to append.
    Args:
...
def connection (points1, paths1, points2, paths2, offset = 0):
    """"
    Given 2 diagrams, it returns all possible products between the 2 diagrams. Note that the order of the diagrams is important
    Args:
        points1:
        paths1:
        points2
